...
5. Apply the database migration sequence in DEPLOYMENT.md to a fresh shop project, then run the generated shop.sql. Preserve the generated UUID. New pricing starts from the repository template; review every rate with the owner.
...
7. Open admin settings, check branding/pricing and merchant mapping; activate only that shop's configured merchant. Static merchant IDs cannot be pasted as substitutes for API credentials.
