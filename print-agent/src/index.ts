    ...
    try {
      ...
      if (currentJob) {
        try {
          ...
          await client.reportJobCompletion(currentJob.order_id, false, errMsg);
          logger.warn(`[JOB:${currentJob.order_number}] Reported failure to backend: ${errMsg}`);
        } catch (reportErr) {
          logger.error('Failed to report job failure status to server:', reportErr);
        }
      }
    } finally {
      isProcessing = false;
