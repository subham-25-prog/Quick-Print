...
export class AgentWorker{
  private busy=false;
  constructor(private client:Pick<ShopApiClient,'claimNextJob'|'downloadDocument'|'startJob'|'reportJobCompletion'>,
    private printer:{ensureReady():Promise<void>;printDocument(file:string,job:ClaimedJob):Promise<void>},private journal:Journal,private dir:string){}
  async tick(){
    ...
    try{
      ...
      const job=await this.client.claimNextJob();if(!job)return;
      if(this.journal.dispatched(job.job_id))throw new Error('Locally dispatched job returned by server; manual recovery required');
      const file=path.join(this.dir,`${randomUUID()}.pdf`);
      let started=false;
      try{
        await this.client.downloadDocument(job,file);
        await this.printer.ensureReady();
        // fsync BEFORE the server dispatch transition and any physical side effect.
        this.journal.append({job,state:'STARTING'});started=true;
        await this.client.startJob(job);
        await this.printer.printDocument(file,job);
      ...
      console.log(JSON.stringify({event:'print_result',jobId:job.job_id,outcome:entry.outcome,test:job.is_test}));
