...
export async function POST(req:NextRequest,{params}:{params:Promise<{id:string}>}){
  try{
    ...
    if(checked.status==='SUCCESS')return NextResponse.json(await openPayment(checked,provider,requestOrigin));
    ...
    if(ce){if(ce.code==='23505'){const {data:active}=await db.from('payments').select('*').eq('uploaded_file_id',p.uploaded_file_id).eq('shop_id',shop).in('status',['PENDING','SUCCESS']).maybeSingle();if(active)return NextResponse.json(await openPayment(active,provider,requestOrigin));}throw ce;}
    return NextResponse.json(await openPayment(created,provider,requestOrigin));
