...
export const AdobePrintPreviewModal: React.FC<AdobePrintPreviewModalProps> = ({
  ...
  // Synchronize internal state when modal opens or parent advancedConfig updates
  useEffect(() => {
    if (!isOpen) return;
    setConfig({
      pageRangeMode: advancedConfig.pageRangeMode || 'ALL',
  ...
  return (
    <div
      role="dialog"
      aria-modal="true"
      aria-label="Adobe Acrobat Advanced Print Settings and Preview"
      className="fixed inset-0 z-50 bg-slate-950/90 backdrop-blur-md flex flex-col h-[100dvh] w-screen overflow-hidden font-sans select-none pb-safe pt-safe"
    >
        ...
        <div className="flex items-center gap-2 text-center min-w-0 px-2">
          ...
          <div className="min-w-0">
            ...
            <div className="text-[10px] text-slate-400 font-mono">
              Adobe Print Preview • Page {currentPage} of {totalDocPages}
            </div>
          </div>
        </div>
          ...
          <div className="md:hidden w-full bg-slate-900 border-t border-slate-800 p-2.5 flex items-center justify-between text-xs text-slate-300 shrink-0 z-30 shadow-lg">
            ...
            >
              <div className="flex items-center gap-2">
                ...
                <span>Adobe Print Settings</span>
              </div>
        ...
        >
          ...
          <div className="space-y-2">
            ...
            {config.pageRangeMode === 'RANGE' && (
              <input
                ...
                className="w-full min-h-[44px] px-3.5 py-2.5 rounded-xl bg-slate-950 border border-slate-800 text-white font-mono text-xs focus:outline-hidden focus:border-red-500 mt-2"
              />
            )}
          </div>
          ...
          <div className="space-y-2 pt-2 border-t border-slate-800">
            ...
            <select
              ...
              className="w-full min-h-[44px] px-3.5 rounded-xl bg-slate-950 border border-slate-800 text-slate-200 text-xs font-bold focus:outline-hidden focus:border-red-500 cursor-pointer"
            >
