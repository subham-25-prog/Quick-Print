...
const migrations=JSON.parse(await readFile(resolve(root,'supabase/migration-order.json'),'utf8'));
const parts=await Promise.all(migrations.map(file=>readFile(resolve(root,'supabase',file),'utf8')));
await writeFile(resolve(dir,'fresh-install.sql'),'-- FRESH SUPABASE PROJECT ONLY. Do not rerun on an existing installation.\n'+parts.join('\n\n')+'\n'+sql,{flag:'wx'});
await writeFile(resolve(dir,'README.txt'),'QuickPrint shop installation\n\n1. Create a NEW Supabase project owned by this shop.\n2. Run fresh-install.sql once in its SQL Editor. Do not also run shop.sql.\n3. Fill Supabase credentials in .env.production.local and import into this shop\'s Vercel Production environment. Never commit the generated folder.\n4. Deploy the repository, then open /admin/setup and review all checks.\n5. Leave PhonePe fields empty until approved credentials are available. Checkout stays disabled.\n6. Configure the print agent from print-agent.env; start with simulation enabled.\n7. Follow docs/NEW_SHOP_INSTALLATION.md for payment activation, maintenance and acceptance tests.\n',{flag:'wx'});
